...
const createProductCards = (products) => {
  ...
  products.forEach((product) => {
    ...
    img.addEventListener('click', () => {
      ...
      try {
        const coords = JSON.parse(product.coords);
        // console.log(coords);
        addMarker(coords);
      } catch (e) {}
    });
    ...
    p1.innerHTML = `Birthdate: ${dt
      .fromISO(product.birthdate)
      .setLocale('fi')
      .toLocaleString()}`;
    const p1b = document.createElement('p');
    p1b.innerHTML = `Age: ${dt
      .now()
      .diff(dt.fromISO(product.birthdate), ['year'])
      .toFormat('y')}`;

    const p2 = document.createElement('p');
    p2.innerHTML = `Weight: ${product.weight}kg`;
    ...
    li.appendChild(p3);
    ul.appendChild(li);
    if (user.role === 0 || user.user_id === product.owner) {
      // add modify button
      ...
      modButton.addEventListener('click', () => {
        ...
        inputs[1].value = product.birthdate;
        inputs[2].value = product.weight;
        modForm.action = `${url}/product/${product.product_id}`;
        if (user.role === 0)
...
// when app starts, check if token exists and hide login form, show logout button and main content, get products and users
(async () => {
